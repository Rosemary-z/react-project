  ...
  getMenuList = (menuList) => {
    ...
    return menuList.map((item) => {
      if (!item.children) {
        ...
        let bool = item.roles.includes(role); // 判断当前用户是否在允许的可访问的roles列表内
        return bool ? (
        ...
        let { role } = this.props.user.userInfo; //一级权限是没有设置role的，直接让用户不能访问
        let bool = cItem.roles.includes(role);
